...
from optim.optim_factory import build_optimizer_for_aux_model
...
parser.add_argument('--color-jitter', type=float, nargs='+', default=[0.4, 0.4, 0.4, 0.0], metavar='VAL',
                    help='Color jitter factors: brightness, contrast, saturation, hue (default: 0.4 0.4 0.4 0.0)')
...
parser.add_argument('--edge-optim', action='store_true', help='Enable edge branch optimizer with different lr')
parser.add_argument('--canny-lr', type=float, default=None, help='Learning rate for canny edge branch optimizer')
...
def main():
    ...
    # CHECKPOINT: deit_aux registered in timm.models
    model = create_model(
        args.model,
    ...
    if args.edge_aux and args.edge_optim:
        args.canny_lr = args.canny_lr or (args.lr * 1e2)# set canny branch lr to 2x of main branch
        optimizer = build_optimizer_for_aux_model(model, args)
    else:
    ...
    dataset_train = create_dataset(
        ...
        batch_size=args.batch_size, repeats=args.epoch_repeats,edge_aux=args.edge_aux)
    dataset_eval = create_dataset(
        args.dataset, root=args.data_dir, split=args.val_split, is_training=False, batch_size=args.batch_size,edge_aux=args.edge_aux)
    ...
    if args.jsd:
        ...
        train_loss_fn = nn.CrossEntropyLoss(weight=torch.tensor([0.016129032258064516, 0.014285714285714285, 0.004739336492890996, 0.0017391304347826088, 0.004132231404958678, 0.004901960784313725, 0.023255813953488372, 0.019230769230769232, 0.00390625, 0.00546448087431694, 0.001, 0.001, 0.003787878787878788, 0.029411764705882353, 0.001, 0.005847953216374269, 0.007692307692307693, 0.013333333333333334, 0.0034602076124567475, 0.004830917874396135, 0.006172839506172839, 0.013333333333333334, 0.007407407407407408, 0.005050505050505051, 0.007575757575757576, 0.007042253521126761, 0.037037037037037035, 0.03333333333333333, 0.001059322033898305, 0.011111111111111112, 0.002881844380403458, 0.005952380952380952])).cuda()
...
def train_one_epoch(
    ...
    for batch_idx, batch in enumerate(loader):
        ...
        if not args.prefetcher:
            ...
            if mixup_fn is not None:
                input_rgb, target = mixup_fn(input_rgb, target)
        ...
        with amp_autocast():
            ...
            if args.edge_aux:
                if args.use_edge_attn:
                    ...
                    loss1 = loss
                    loss2 = torch.tensor(0., device=loss.device)
                elif args.use_edge_fusion:
                    output = model(input_rgb, input_edge, use_fusion=args.use_edge_fusion)
                    loss = loss_fn(output, target)
                    loss1 = loss
                    ...
                    loss2 = loss_fn(edge_output, target)
                    loss = loss1 + args.edge_aux_weight * loss2
            else:
                output = model(input_rgb)
                loss = loss_fn(output, target)
                loss1 = loss
        ...
        if not args.distributed:
            losses_m.update(loss.item(), input_rgb.size(0))
            losses1_m.update(loss1.item(), input_rgb.size(0))
            losses2_m.update(loss2.item(), input_rgb.size(0))
        ...
        if last_batch or batch_idx % args.log_interval == 0:
            ...
            if args.distributed:
                ...
                reduced_loss1 = reduce_tensor(loss1.data, args.world_size)
                losses1_m.update(reduced_loss1.item(), input.size(0))
                reduced_loss2 = reduce_tensor(loss2.data, args.world_size)
                losses2_m.update(reduced_loss2.item(), input.size(0))
            ...
            if args.rank == 0:
                _logger.info(
                    ...
                    'Data: {data_time.val:.3f} ({data_time.avg:.3f})'.format(
                        ...
                        rate=input_rgb.size(0) * args.world_size / batch_time_m.val,
                        rate_avg=input_rgb.size(0) * args.world_size / batch_time_m.avg,
                        lr=lr,
    ...
    return OrderedDict([('loss', losses_m.avg),('loss1',losses1_m.avg), ('loss2',losses2_m.avg)])
...
def validate(model, loader, loss_fn, args, amp_autocast=suppress, log_suffix=''):
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(loader):
            ...
            with amp_autocast():
                # CHECKPOINT: return both
                if args.edge_aux:
                    if args.use_edge_attn:
                        output = model(input_rgb, input_edge)
                    elif args.use_edge_fusion:
                        output = model(input_rgb, input_edge, use_fusion=args.use_edge_fusion)
                    else:
                        output, edge_output = model(input_rgb, input_edge)
            ...
            if edge_output is None:
                ...
                loss = loss_fn(output, target) + args.edge_aux_weight * loss_fn(edge_output, target)
            ...
            losses_m.update(reduced_loss.item(), input_rgb.size(0))
